...
class Paddy extends CI_Model {
    ...
    public function get_transaction_type($soc_id,$trans_dt,$bulk_trans_id){

        $kms_id = $this->session->userdata['loggedin']['kms_id'];
        $sql="select trans_type from td_collections WHERE kms_id = '$kms_id' and soc_id = '$soc_id' and trans_dt = '$trans_dt' and bulk_trans_id = '$bulk_trans_id' ";
    ...
    public function f_forward_reissue_paddycollection($trans_dt,$bulk_trans_id,$soc_id){
        ...
        $sql="UPDATE `td_collections` SET `status` = '1',`dwn_flag` = '0' WHERE trans_dt = '$trans_dt' AND `bulk_trans_id` = $bulk_trans_id AND `soc_id` = $soc_id";
        $this->db->query($sql);
    }
    public function f_forward_neft($trans_dt,$bulk_trans_id,$soc_id){

        $sql="UPDATE `td_collections` SET `status` = '1',`dwn_flag` = '0', `chq_status` = 'U' WHERE trans_dt = '$trans_dt' AND `bulk_trans_id` = $bulk_trans_id AND `soc_id` = $soc_id";
        $this->db->query($sql);
    }
